...
export default function Installation({
  ...
  return (
    <>
      <h6 className="mt-10 mb-2 text-[20px] ml-1 font-semibold">
        Installation
      </h6>
      ...
      <div className="h-fit w-80 md:w-136 mt-10">
        {isDefault ? (
          <CommandBlock CLILink={CLILink} />
        ) : (
          <ManualSection path={path} code={code} />
        )}
      </div>
    </>
  );
...
function CommandBlock({ CLILink }: { CLILink: string }) {
  ...
  useEffect(() => {
    const activeTitle = CommandBlockItems.find(
      (i) => i.cmd === currentCmd,
    )?.title;
  ...
  return (
    <div
      className="h-26 w-80 md:w-172 pb-1 md:pb-2 bg-neutral-200/40 border border-neutral-200
      dark:border-neutral-800 dark:bg-neutral-800/50 rounded-lg"
    >
  ...
  return (
    <div>
      <div>
        ...
        <div className="not-prose bg-neutral-200 dark:bg-neutral-800 text-black dark:text-white/80 p-3 rounded-md flex items-center justify-between md:w-172">
      ...
      <div className="mt-10">
        ...
        <div className="md:w-172 h-58 bg-neutral-200 rounded-xl dark:bg-neutral-800">
          <div className="p-1 px-4 text-neutral-600 dark:text-neutral-400 tracking-wide font-mono text-[14px]">
            lib/utils.ts
          </div>
          <div className="mx-2 bg-white rounded-lg h-48 dark:bg-neutral-900 shadow-[0_3px_10px_rgb(0,0,0,0.2)]">
            <CodeBlock name="clsx" />
          </div>
        </div>
      </div>
      ...
      <div className="mt-10">
        ...
        <div className="md:w-172 h-98 bg-neutral-200 rounded-xl dark:bg-neutral-800">
          <div className="p-1 px-4 text-neutral-600 dark:text-neutral-400 tracking-wide font-mono text-[14px]">
            {path}
          </div>
          <div className="mx-2 bg-white rounded-lg h-88 dark:bg-neutral-900 shadow-[0_3px_10px_rgb(0,0,0,0.2)]">
            <CodeBlock name={code} />
          </div>
        </div>
      </div>
